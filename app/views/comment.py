...
class CommentView(FlaskView):
    def index(self, post):
        Post.objects.get_or_404(id=post)
        comments = Comment.objects(post=post).order_by('-num_of_likes')
        return jsonify(get_paginated_list(
            model='comments', results=comments, schema=CommentSchema(),
            url=f'/posts/{post}/comments', params='',
    ...
    def post(self, post):
        ...
        comment = Comment(user=get_jwt_identity(), post=post, **data)
        comment.save()
        return CommentSchema().dump(comment), 201

    @jwt_required
    def patch(self, post, id):
...
class RecommentView(FlaskView):
    ...
    def post(self, post, comment):
        try:
            ...
            return err.messages, 400
        recomment = Recomment(user=get_jwt_identity(), comment=comment, **data)
        recomment.save()
        return RecommentSchema().dump(recomment), 201

    def patch(self, post, comment, id):
    ...
    def delete(self, post, comment, id):
        recomment = Recomment.objects.get_or_404(id=id)
        user = User.objects.get(id=get_jwt_identity())
        if recomment.user != user:
            return {'message': '권한이 없습니다'}, 403
